...
public class BlogController {
   private final PostRepository postRepository;
    ...
    //можно в форме на отправке в пост сделать отправку объекта сразу
    // и не париться с приемкой параметров, но я выполняю обучалку
    //делаю как автор
    ...
    public String blogDetails(@PathVariable(value = "id") long id,  Model model)
    {
        if (postRepository.existsById(id))
            {
                Optional<Post> post = postRepository.findById(id); // Optional<Post> post = postRepository.findAllById(id);
